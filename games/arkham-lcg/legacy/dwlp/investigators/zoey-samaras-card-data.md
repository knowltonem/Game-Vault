# Zoey Samaras — Card Data Reference

---

## Collection
| Field | Value |
|---|---|
| Set | The Dunwich Legacy Investigator Expansion (dwlp) |
| Card Number | 02001 |

## Investigator (FRONT)
| Field | Value |
|---|---|
| Name | Zoey Samaras |
| Subtitle | The Chef |
| Class | Guardian |
| Traits | *Believer. Hunter.* |
| Willpower | 4 |
| Intellect | 2 |
| Combat | 4 |
| Agility | 2 |
| Health | 9 |
| Sanity | 6 |
| Ability text | <reaction> After you become engaged with an enemy: Gain 1 resource.
<elder_sign> effect: +1. If this skill test is successful during an attack, that attack deals +1 damage. |
| Flavor text | *"God has spoken. I will do His work without hesitation."* |
| Collection | 02001 |

## Investigator (BACK)
| Field | Value |
|---|---|
| Name | Zoey Samaras |
| Subtitle | The Chef |
| Class | Guardian |
| Deck Size | 30 |
| Deckbuilding Options |   • Guardian, Neutral (level 0–5)<br>  •  (level 0–0) |
| Deckbuilding Requirements | Zoey's Cross (× 1), Smite the Wicked (× 1), 1 random Basic Weakness |
| Flavor text | *Zoey had known that she was special ever since God spoke to her one night when she was six years old... the night that terrible fire took away her parents. He told her that He had chosen her from among all the people of the world to be His agent. She would protect the innocent and punish the wicked. Since then, He comes to her in times of trouble, offering guidance and comfort. Zoey now travels from city to city, taking work as a chef to support herself. When she isn't working, she stalks the night, guided by the Lord's voice. Wherever she finds wickedness, she strikes it down without remorse or hesitation.* |

## Elder Sign
effect: +1. If this skill test is successful during an attack, that attack deals +1 damage.

## Signature Asset — Zoey's Cross (Symbol of Righteousness) (02006)
| Field | Value |
|---|---|
| Type | Asset |
| Traits | *Item. Charm.* |
| Cost | 1 |
| Slot | Accessory |
| Text | Zoey Samaras deck only.
<reaction> After an enemy becomes engaged with you, exhaust Zoey's Cross and spend 1 resource: Deal 1 damage to that enemy. |
| Skill Icons | <com> <com> <wild> |

## Personal Weakness (02007)
| Field | Value |
|---|---|
| Type | Treachery (Weakness) |
| Traits | *Task.* |
| Text | <b>Revelation</b> - Discard cards from the top of the encounter deck until an enemy is discarded. Attach Smite the Wicked to that enemy and spawn it at the location farthest from you.
<b>Forced</b> - When the game ends, if attached enemy is in play: Zoey Samaras suffers 1 mental trauma. |

...
MODE="${1:-verify}"
baseline(){ ( cd "$ORIG" && find . -type f -not -path "./Card Data - Original/*" -not -name ".original-checksums.sha256" -print0 | sort -z | xargs -0 sha256sum > ".original-checksums.sha256" ); echo "baseline written ($(grep -c . "$ORIG/.original-checksums.sha256") files)"; }
...
lock(){ find "$ORIG" -type f -not -path "*/Card Data - Original/*" -exec chmod a-w {} +; echo "locked"; }

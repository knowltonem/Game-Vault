# Helicopter-Pad → Ice Runway

**Source:** Original at `outpost31/Outpost 31 (Arkham Central)/Full Bleed Files/Locations/Helicopter-Pad-Front-Face.png`

**Mod status:** 🟡 QUEUED — data received, ready for Strange Eons

**Illustrator:** FrogmanDev

**Encounter set:** Snowflake (❄) | **Number:** 9 | **Set:** II

---

## Original Card Data

### Front Face (Revealed)

| Field | Original | Modified |
|---|---|---|
| Card Name | Helicopter Pad | **Ice Runway** |
| Ghost title | "Helicopter Bay" (distorted artifact) | Remove |
| Type | Location | — |
| Trait | Facility. | — |
| Shroud | 3 | — |
| Clues | 3 | — |
| Per investigator | No | — |
| Victory | None | — |
| Own symbol | Red slash/lozenge ♦ | — |
| Connection 1 | Magenta spade → Vehicle Storage | — |
| Connection 2 | Green T/tee → Supply Room | — |

**Ability 1 (Forced):**
> After you fail a skill test while investigating Helicopter Pad: Move 1 of your clues to the nearest Crew Member.

→ Modified: replace "Helicopter Pad" with **"Ice Runway"**

**Ability 2 (granted action):**
> While there are no enemies at Helicopter Pad (and it has no damage), it gains [action]: Resign. You flee to safety.

→ Modified: replace "Helicopter Pad" with **"Ice Runway"**

### Back Face (Unrevealed)

**Flavor text:**
> The station's primary link to the outside world for personnel and supply transfers. Weather permitting, flights arrive weekly with fresh provisions.

→ No change needed.

---

## Art Notes

The art likely shows a helicopter landing pad. Options:
- Leave as-is (mentally re-flavor to ski-plane ice runway)
- Generate new art via Midjourney
- Use Krea AI to inpaint

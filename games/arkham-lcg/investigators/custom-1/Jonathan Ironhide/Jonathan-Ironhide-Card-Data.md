# Jonathan Ironhide — Investigator Pack (EON VERIFIED)

← [[games/arkham-lcg/investigators/custom-1/index|Back to Custom Investigators]]

---

## Investigator Card

| Field | Value |
|---|---|
| **Name** | Jonathan Ironhide |
| **Subtitle** | The R'lyeh Survivor |
| **Class** | Guardian |
| **Traits** | Hunter. Cursed. |
| **Willpower** | 3 |
| **Intellect** | 2 |
| **Combat** | 5 |
| **Agility** | 3 |
| **Health** | 9 |
| **Sanity** | 7 |
| **Deckbuilding** | Guardian, Neutral (level 0-5). Mystic (level 0-2). Survivor (level 0-1). |

### Ability
[rea] After you defeat an enemy: Gain 1 resource and deal 1 damage to each enemy at your location. (Limit once per round.)

### Elder Sign
+1. If you succeed, draw 1 card and gain 1 resource.

### Deckbuilding Requirements (do not count toward deck size)
- The Hollow Warden ×1
- Hydra Hyde ×1
- Echoes of R'lyeh ×1
- 1 random Basic Weakness

### Deck Size
30.

### Back Flavor
*"I've seen the sunken city, I can't get it off me."*

### Flavor
*"Let's Dance."*

---

## Signature Cards (shuffled in, don't count toward deck)

### 002 The Hollow Warden

| Field | Value |
|---|---|
| **Subtitle** | Unholy |
| **Type** | Asset — Hand |
| **Slot** | Hand |
| **Traits** | Item. Weapon. Firearm. Relic. |
| **Cost** | 0 |
| **Uses** | 3 ammo |

### Text
Uses (3 ammo).

[act] Spend 1 ammo: [com] Fight. You get +2 [com] and deal +1 damage for this attack. If this attack defeats an enemy, draw 1 card.

[act] Spend 2 resources: Add 2 ammo to The Hollow Warden.

[fre] At the start of your turn, if The Hollow Warden has 0 ammo: Add 1 ammo to it.

### Flavor
*"It was remade in R'lyeh."*

---

### 003 Hydra Hyde

| Field | Value |
|---|---|
| **Subtitle** | Hide of the Deep Mother |
| **Type** | Asset — Body |
| **Slot** | Body |
| **Traits** | Item. Armor. Relic. |
| **Cost** | 2 |
| **HP Soak** | 3 |
| **SAN Soak** | 3 |

### Text
Reduce damage or horror dealt to you by 1.

If Hydra Hyde would be discarded, you may spend 2 resources to shuffle it into your deck.

### Flavor
*"A gift from the Deep."*

---

### 004 Echoes of R'lyeh

| Field | Value |
|---|---|
| **Type** | Treachery — Weakness |
| **Traits** | Flaw. Curse. |

### Text
[rev] Take 2 horror.

[for] After you defeat an enemy: Take 1 horror.

[act] Spend 4 resources: Discard Echoes of R'lyeh.

### Flavor
*"This madness consumes me."*

---

## Deck (005-034 = 30 cards)

| # | Card | Type | Cost | Traits | Rules | Flavor |
|---|---|---|---|---|---|---|
| 005-006 | King's Talon ×2 | Asset — Hand | 2 | Item. Weapon. Relic. Cursed. | [act] [com] Fight. +2 [com] and +1 damage. If [cur] token revealed: deal +2 damage instead. If attack defeats enemy: heal 1 horror. You get +1 additional [com] per damage on attacked enemy (max +3). | "Hastur's servant fell screaming. Ironhide kept the claw." |
| 007-008 | Hound of the Deep ×2 | Asset — Ally | 3 | Ally. Creature. Cursed. | 3/1 soak. Icons: agi/agi. You get +1 [com]. [rea] After an enemy attacks you: Exhaust — deal 1 damage to that enemy. | "It follows. It always follows." |
| 009-010 | Luck of the Draw ×2 | Event | 1 | Insight. | Fast. INT/INT/INT icons. When you would discover 1 clue, instead discover 2 clues. | "Some men prepare. I just show up." |
| 011-012 | Father Thomas ×2 | Asset — Ally | 3 | Ally. Priest. | 2/2 soak. You get +1 [com]. [act] Exhaust: Deal 1 damage to an enemy at your location, or discover 1 clue at your location. | "In my vision I saw the truth of the old one, but my faith holds strong." |
| 013-014 | R'lyeh Fury ×2 | Skill | — | Practiced. | If successful during a [com] action: deal +1 damage. | "I don't fight to win. I fight to end it." |
| 015-016 | I'll See You in Hell ×2 | Event | 1 | Tactic. | Deal 1 damage to an enemy at your location. You take 1 damage. [fre] If this card just defeated an enemy: Draw 2 cards. | "I've stared into the abyss. It blinked first." |
| 017 | Cosmic Blast ×1 | Event | 5 | Tactic. | Deal 3 damage to each enemy at a location within 2 connections. Each investigator at that location takes 1 damage. | "I am the flame." |
| 018 | On the Hunt ×1 | Event | 1 | Tactic. | Search the encounter deck and discard pile for an enemy of your choice and spawn it engaged with you. Shuffle the encounter deck. Draw 1 card. | "I don't wait for the dark to find me. I go looking for it." |
| 019-020 | Antique Dealings ×2 | Event | 0 | Supply. | Gain 3 resources. | "The relic fetched a good price. Don't ask where it came from." |
| 021-022 | Premonitions ×2 | Event | 1 | Tactic. Supply. | Search the top 9 cards of your deck for a Weapon asset and draw it. Shuffle your deck. | "I saw the fight before it happened." |
| 023-024 | Knowledge From the Deep ×2 | Event | 1 | Insight. | Choose an investigator at your location. That investigator heals 2 horror. | "Unholy healing for unholy wounds." |
| 025 | I'll Manage ×1 | Skill | — | Innate. | If this skill test fails: Gain 1 resource and draw 1 card. | "I've been on my own for so long." |
| 026 | Aquinnah ×1 | Asset — Ally | 3 | Ally. Seer. | 1/3 soak. [rea] After an enemy attacks you and deals damage: Deal that much damage to another enemy at your location. | "She sees things before they happen. She never looks happy about it." |
| 027-028 | Ward of Protection ×2 | Event | 1 | Spell. Spirit. | Cancel the effects of a non-weakness encounter card just drawn. Take 1 horror. | "Against the darkness, even a small light can hold back the tide." |
| 029-030 | Holy Water ×2 | Asset — Hand | 2 | Item. Relic. | Uses (4 charges). Spend 1 charge: Add 1 [bless] token to the chaos bag. Spend 1 charge: Heal 1 horror from an investigator at your location. | "Blessed by the faithful. Deadly to the damned." |
| 031-032 | Overpower ×2 | Skill | — | Practiced. | [com] icon ×2. | "My Power Comes From Within." |
| 033-034 | Guts ×2 | Skill | — | Innate. | If this skill test is successful: Draw 1 card. | "Fear is a luxury I can't afford." |

---

## Pack Information

| Field | Value |
|---|---|
| **Pack Name** | Jonathan Ironhide Investigator Pack |
| **Pack Code** | RYP-JI |
| **Total Cards** | 034 |
| **Investigator** | 001 Jonathan Ironhide |
| **Signature Asset 1** | 002 The Hollow Warden (shuffled in) |
| **Signature Asset 2** | 003 Hydra Hyde (shuffled in) |
| **Signature Weakness** | 004 Echoes of R'lyeh (shuffled in) |
| **Deck** | 005-034 (30 cards) |

---

## Slot Picture

| Slot | Card |
|---|---|
| Hand 1 | The Hollow Warden |
| Hand 2 | King's Talon / Enchanted Blade / Holy Water |
| Body | Hydra Hyde |
| Ally 1 | Hound of the Deep / Father Thomas / Aquinnah |
| Ally 2 | (open) |

---

## Key Design Decisions — LOCKED (EON VERIFIED)

| Decision | Value |
|---|---|
| Stats | WIL 3 / INT 2 / COM 5 / AGI 3 / HP 9 / SAN 7 |
| Ability | [rea] After defeat enemy: Gain 1r + deal 1 dmg to each enemy at location (once per round) |
| Elder Sign | +1. Draw 1 card and gain 1 resource on success |
| Hollow Warden | Cost 0, Hand, 3 ammo, +2 COM +1 dmg, draw on kill, reload 2r=2 ammo, free reload 1 ammo on 0 |
| Hydra Hyde | Cost 2, Body, 3/3 soak, reduce dmg or hor by 1, spend 2r to shuffle back on discard |
| Echoes of R'lyeh | Weakness, revelation 2 horror, after kill 1 horror, spend 4r to discard |
| King's Talon | Cost 2, Hand, +2 COM +1 dmg, [cur]=+2 dmg, kill=heal 1 hor, +1 COM per dmg on target (max +3) |
| Hound of the Deep | Cost 3, Ally, 3/1 soak, +1 COM, exhaust deal 1 dmg to attacking enemy |
| Enchanted Blade | Cost 3, Hand, 3 charges, +1 COM base, spend charge: +2 more COM + 1 dmg on success |
| Father Thomas | Cost 3, Ally, 2/2 soak, +1 COM, exhaust: 1 dmg OR 1 clue |
| R'lyeh Fury | Skill, practiced, +1 dmg on successful COM action |
| I'll See You in Hell | Cost 1, deal 1 dmg take 1 dmg, kill=draw 2 cards |
| Cosmic Blast | Cost 5, 3 dmg to all enemies within 2 connections, investigators take 1 dmg |
| On the Hunt | Cost 1, spawn chosen enemy from encounter deck engaged with you, draw 1 |
| Antique Dealings | Cost 0, gain 3 resources |
| Premonitions | Cost 1, search top 9 for Weapon asset |
| Knowledge From the Deep | Cost 1, target investigator heals 2 horror |
| Aquinnah | Cost 3, Ally, 1/3 soak, redirect incoming damage to another enemy |
| Holy Water | Cost 2, Hand, 4 charges, add bless OR heal 1 horror |
| Overpower | Skill, practiced, COM ×2 icons |
| I'll Manage | Skill, innate, fail=gain 1r + draw 1 |

---

## ⚠️ Items Requiring Attention

| Item | Issue |
|---|---|
| 002-006 PNGs | Missing — needs Strange Eons export |
| 001 | Needs regen (ability/elder sign text may be outdated) |
